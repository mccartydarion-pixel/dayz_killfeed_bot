...
func TestLinkErrorMessageDistinguishesOutcomes(t *testing.T) {
	cases := []struct {
		...
		{linking.ErrInstallationNotConfigured, "SERVER NOT CONFIGURED"},
		{linking.ErrServerSelectionRequired, "SERVER SELECTION REQUIRED"},
	...
	for _, err := range []error{linking.ErrInstallationNotConfigured, linking.ErrServerSelectionRequired, linking.ErrPlayerNotObserved, linking.ErrInvalidUsername} {
