...
func TestGameserverLiveReadsQueryPlayerCounts(t *testing.T) {
	c := liveTestClient(t, 200, `{"status":"success","data":{"gameserver":{"service_id":19806451,"status":"started","slots":18,
	...
	if !live.BelongsTo("19806451") || live.BelongsTo("19806452") {
		t.Fatalf("service ownership check failed: %+v", live)
	}
}

// A response that does not name its service cannot be proven to be ours.
func TestGameserverLiveWithoutServiceIDDoesNotBelong(t *testing.T) {
	c := liveTestClient(t, 200, `{"data":{"gameserver":{"status":"started","slots":18,"query":{"player_current":3,"player_max":18}}}}`)
	live, err := c.GameserverLive(context.Background(), "19806451")
	if err != nil {
		t.Fatal(err)
	}
	if live.BelongsTo("19806451") {
		t.Fatal("a response without service_id must not be attributed to the requested service")
	}
}
