...
## Zones + UAV / Base Radar (migration 0042)

See `docs/ZONES_UAV_RADAR.md` for the full design. Six new tables:

| Table | Notes |
|---|---|
| `installation_zones` | `installation_id` (tenant-CRUD identity) plus denormalized `guild_id`/`server_id` (resolved once at creation, mirroring `player_location_events` - the intrusion engine in `internal/killfeed` only ever knows guild/server ids). `zone_type` (`CHECK IN` SAFEZONE, PVP, RESTRICTED, EVENT, UAV, BASE_RADAR, CUSTOM; immutable after creation), `center_x`/`center_z`/`radius` (`DOUBLE PRECISION`, `radius > 0`), `alert_channel_id` (nullable - no alert is ever sent when unset, no fallback channel), `cooldown_seconds` (default 300), `enabled`. Indexes `(installation_id)`, `(server_id, enabled)` (backs the zone cache's refresh query) |
| `zone_ignore_entries` | `zone_id` (`ON DELETE CASCADE`), `entry_type` (`CHECK IN` PLAYER, FACTION, DISCORD_ROLE), `entry_value`, `UNIQUE(zone_id, entry_type, entry_value)`. A matching entity is never tracked at all - no presence row, no intrusion, no alert |
| `zone_authorized_entries` | Same shape as `zone_ignore_entries` but `entry_type CHECK IN` (PLAYER, FACTION) only - meaningful only for UAV/BASE_RADAR zones (suppresses alerting, not tracking) |
